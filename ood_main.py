...
from core import *
from misc import *
from datasets.osr_loader import CIFAR10_OSR, CIFAR100_OSR, SVHN_OSR, Tiny_ImageNet_Crop, Tiny_ImageNet_Resize, LSUN_Crop, LSUN_Resize
...
def getLoader(options):
    ...
    if 'svhn' in options['dataset']: # ood main SVHN_OSR changed like cifar100
        options['img_size'] = 32
        ...
        out_loader = out_Data.test_loader
    elif 'tiny_imagenetC' in options['dataset']:
        options['img_size'] = 32
        options['patch_size'] = 8  # For ViT
        Data = CIFAR10_OSR(known=options['known'], batch_size=options['batch_size'], img_size=options['img_size'], options=options)
        train_loader, test_loader = Data.train_loader, Data.test_loader
        out_Data = Tiny_ImageNet_Crop(batch_size=options['batch_size'], img_size=options['img_size'], options=options)
        out_loader = out_Data.test_loader
    elif 'tiny_imagenetR' in options['dataset']:
        options['img_size'] = 32
        options['patch_size'] = 8  # For ViT
        Data = CIFAR10_OSR(known=options['known'], batch_size=options['batch_size'], img_size=options['img_size'], options=options)
        train_loader, test_loader = Data.train_loader, Data.test_loader
        out_Data = Tiny_ImageNet_Resize(batch_size=options['batch_size'], img_size=options['img_size'], options=options)
        out_loader = out_Data.test_loader
    elif 'LSUNC' in options['dataset']:
        options['img_size'] = 32
        options['patch_size'] = 8  # For ViT
        Data = CIFAR10_OSR(known=options['known'], batch_size=options['batch_size'], img_size=options['img_size'], options=options)
        train_loader, test_loader = Data.train_loader, Data.test_loader
        out_Data = LSUN_Crop(batch_size=options['batch_size'], img_size=options['img_size'], options=options)
        out_loader = out_Data.test_loader
    elif 'LSUNR' in options['dataset']:
        options['img_size'] = 32
        options['patch_size'] = 8  # For ViT
        Data = CIFAR10_OSR(known=options['known'], batch_size=options['batch_size'], img_size=options['img_size'], options=options)
        train_loader, test_loader = Data.train_loader, Data.test_loader
        out_Data = LSUN_Resize(batch_size=options['batch_size'], img_size=options['img_size'], options=options)
        out_loader = out_Data.test_loader
...
def main(options):
    ...
    for i in range(len(splits[options['dataset']])):
        ...
        if options['dataset'] == 'cifar100':
            known = splits['cifar10'][i]
            unknown = splits[options['dataset']][i] # for cifar+10 and cifar+50 exp
        elif options['dataset'] == 'svhn':
            known = splits['cifar10'][i]
            unknown = splits[options['dataset']][i]
        else:
            known = splits['cifar10'][i]
            unknown = splits[options['dataset']][i]
...
if __name__ == '__main__':
    ...
    options['dataset'] = 'LSUNR'#'svhn/tiny_imagenet/tiny_imagenetC/R/LSUNC/R'
