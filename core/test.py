...
def evaluation(model, test_loader, out_loader, **options):
    ...
    fpr_at_tpr95 = fpr[np.abs(np.array(tpr) - 0.95).argmin()]
    ...
    print('Accuracy (%): {:.3f}, '  .format(acc), 
          ...
          'DTACC: {:.5f}'           .format(det_acc),
          'FPR@TPR95: {:.5f}'.format(fpr_at_tpr95),
          )
    
    return acc, auroc, aupr_in, aupr_out, macro_f1, det_acc
